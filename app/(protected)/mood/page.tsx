...
import { Calendar, Clock, Search, SmilePlus, Trash2 } from "lucide-react";
...
export default function MoodPage() {
  ...
  const deleteMoodEntry = async (entry: MoodEntry) => {
    ...
    try {
      const res = await AxiosInstance.delete(API.MOODS.DELETE(entry._id));
      if (!res.data?.success) {
        toast.error(res.data?.message || "Failed to delete mood entry");
        return;
      }

      toast.success("Mood entry deleted");
      setEntries((prev) => prev.filter((e) => e._id !== entry._id));
      setDateResult((prev) => (prev?._id === entry._id ? null : prev));
      setOverviewLoading(true);
      await fetchOverview();
    } catch (e: any) {
      console.error(e);
      const msg = e?.response?.data?.message;
      toast.error(msg || "Failed to delete mood entry");
    }
  };
            ...
            <div style={{ background: C.card, borderRadius: "20px", padding: "24px", border: `1px solid ${C.border}`, boxShadow: "0 2px 10px rgba(30,58,47,0.06)" }}>
              ...
              {dateResult ? (
                <div style={{ display: "flex", gap: "14px", alignItems: "center", padding: "16px 18px", background: `${getMoodColor(dateResult.mood)}08`, borderRadius: "14px", marginBottom: "20px", borderLeft: `4px solid ${getMoodColor(dateResult.mood)}` }}>
                  ...
                  <button
                    type="button"
                    onClick={() => deleteMoodEntry(dateResult)}
                    style={{ width: 34, height: 34, borderRadius: 10, border: "1px solid rgba(239,68,68,0.2)", background: "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#ef4444", flexShrink: 0 }}
                  ...
                  {entries.map((entry) => (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px" }}>
                        ...
                        <button
                          type="button"
                          onClick={() => deleteMoodEntry(entry)}
                          style={{ width: 32, height: 32, borderRadius: 10, border: "1px solid rgba(239,68,68,0.2)", background: "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#ef4444", flexShrink: 0 }}
                          aria-label="Delete"
                          title="Delete"
                        >
